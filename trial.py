...
def fetch_all_questions(lokNo=18, sessionNo=4, max_pages=1000, page_size=10, locale="en"):
    # The loop will fetch pages until the API returns no more data (i.e., `data` is empty or 
    # `questions` list is empty after processing the response). 
    # `max_pages` serves as a maximum safeguard.
    all_questions = []
    headers = {
...
def create_chroma_db(records):
    ...
    # This will create embeddings and store them in the specified directory.
...
def main():
    ...
    try:
        if not st.session_state.db:
            ...
            if os.path.isdir(CHROMA_DB_PATH) and os.listdir(CHROMA_DB_PATH): # Check if directory exists and is not empty
